...
interface IThemelioBridge {
    ...
    event StakesVerified(bytes32 keccakStakesHash, bytes32 blake3StakesHash);
    event TokensBurned(bytes32 indexed themelioRecipient);
    event TokensMinted(address indexed account, uint256 indexed value);
    ...
    function burn(address account_, uint256 id_, uint256 value_, bytes32 themelioRecipient_) external;
    function burnBatch(address account_, uint256[] memory ids_, uint256[] memory values_, bytes32 themelioRecipient_) external;
    function headerLimbo(bytes32) view external returns (uint128 votes, uint64 bytesVerified, uint64 stakeDocIndex);
    function headers(uint256) view external returns (bytes32 transactionsHash, bytes32 stakesHash);
    ...
    function spends(bytes32) view external returns (bool);
    ...
    function verifyHeader(uint256 verifierHeight_, bytes memory header_, bytes memory stakes_, bytes32[] memory signatures_, bool firstTime_) external returns (bool);
    function verifyStakes(bytes memory stakes_) external returns (bool);
    function verifyTx(bytes memory transaction_, uint256 txIndex_, uint256 blockHeight_, bytes32[] memory proof_) external returns (bool);
}
