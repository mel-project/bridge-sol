...
contract ThemelioBridge is ERC20 {
    ...
    *      matches the Merkle root of its corresponding header (at block `blockHeight_`), then the
    *      transaction has been validated and the 'value' field of the transaction is extracted from 
    *      the first output of the transaction and the recipient is extracted from the
